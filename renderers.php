...
class theme_nau_core_renderer extends core_renderer {
    ...
    public function footer() {
        ...
        if (defined('MDL_PERF') || (!empty($CFG->perfdebug) and $CFG->perfdebug > 7)) {
            ...
            if (defined('MDL_PERFTOFOOT') || debugging() || $CFG->perfdebug > 7) {
                $performanceinfo = theme_nau_performance_output($perf);
            }
        }
    ...
    public function navigation_menu() {
        ...
        if (isloggedin() && !isguestuser()) {
            ...
            if (!empty($PAGE->theme->settings->enableevents)) {
                $branchtitle = get_string('events', 'theme_nau');
            ...
            if (!empty($PAGE->theme->settings->enablemysites)) {
                $branchtitle = get_string('mysites', 'theme_nau');
                ...
                if ($sortedcourses) {
                    ...
                    $noenrolments = get_string('noenrolments', 'theme_nau');
            ...
            if (!empty($PAGE->theme->settings->enablethiscourse)) {
                if (ISSET($COURSE->id) && $COURSE->id > 1) {
                    $branchtitle = get_string('thiscourse', 'theme_nau');
                    ...
                    $data = theme_nau_get_course_activities();
    ...
    public function tools_menu() {
        ...
        if (!empty($PAGE->theme->settings->toolsmenu)) {
            $custommenuitems .= "<i class='fa fa-wrench'> </i>".get_string('toolsmenulabel', 'theme_nau')."|#|".
                    get_string('toolsmenulabel', 'theme_nau')."\n";
    ...
    public function naublocks($region, $classes = array(), $tag = 'aside') {
...
class theme_nau_core_course_renderer extends core_course_renderer {
    protected function coursecat_coursebox(coursecat_helper $chelper, $course, $additionalclasses = '') {
        global $CFG, $OUTPUT;
        $type = theme_nau_get_setting('frontpagerenderer');
    ...
    protected function coursecat_coursebox_content(coursecat_helper $chelper, $course, $type=3) {
        ...
        foreach ($course->get_course_overviewfiles() as $file) {
            ...
            $coursename = $chelper->get_course_formatted_name($course);
            if (ISSET($coursename)) {
            $content .= html_writer::tag('h3', html_writer::tag('b', $coursename));
        }
            ...
            if ($isimage) {
                if ($type == 1) {
                    ...
                    $contentimages .= "<div class='cimbox' style='background: #FFF url($url) no-repeat center center; -webkit-background-size: cover;
  ...
  background-size: cover;'></div>";
        ...
        if ($type == 2) {
            $content .= html_writer::start_tag('div', array('class' => 'go'));
            $content .= html_writer::link(new moodle_url('/course/view.php', array('id' => $course->id)),
                    $coursename, array('class' => $course->visible ? '' : 'dimmed', 'title' => $coursename));
            $content .= html_writer::end_tag('div'); // end go.
        }
